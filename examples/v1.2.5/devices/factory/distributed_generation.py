...
class DeviceExtension:
    ...
    def post(self):
        ...
        self.current_command_frames = {
            widget.ui.command.signal_object.frame
            for widget in (
                self.dash.real_current_command,
                self.dash.reactive_current_command,
            )
        }
        ...
        self.dc_control_command_frames = {
            widget.ui.command.signal_object.frame
            for widget in (
                self.dash.dc_current_limit_command,
                self.dash.dc_voltage_limit_command,
            )
        }
    ...
    def combo_changed(self, text):
        ...
        for (
            frame
        ) in self.dc_control_command_frames:  # Added new for loop for the 3rd option.
